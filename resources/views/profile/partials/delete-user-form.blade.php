<section class="space-y-6">
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __('Atenção: Esta ação é permanente. Ao deletar sua conta, todos os seus dados serão removidos de forma definitiva e não poderão ser recuperados. Tem certeza de que deseja continuar?.') }}
        </p>
    </header>
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <h2 class="text-lg font-medium text-gray-900">
                {{ __('Tem certeza que deseja realizar essa ação?') }}
            </h2>
